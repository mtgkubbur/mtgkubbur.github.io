...
//| label: player-summary
{
  ...
  return html`<div class="player-summary">
    ...
    ${best ? html`<div class="stat">
      ...
      <div class="stat-label">Sterkasti kubbur</div>
    </div>` : html``}
  </div>`;
}
```
...
//| label: charts-container
{
  ...
  const eloChart = Plot.plot({
    width: width, height: 340, marginLeft: 55, marginBottom: 35,
    style: chartStyle,
    x: xConfig,
  ...
  const winrateChart = Plot.plot({
    width: width, height: 280, marginLeft: 55, marginBottom: 35,
    style: chartStyle,
    x: xConfig,
    y: { label: "Sigurhlutfall (%)", domain: [0, 100], grid: true },
    marks: [
      Plot.ruleY([50], { stroke: "#d4cfc9", strokeDasharray: "4,4" }),
      Plot.lineY(playerData, { x: "date", y: "winRate", stroke: "#00733e", strokeWidth: 2, curve: "monotone-x" }),
  ...
  const placementChart = Plot.plot({
    ...
    marks: [
      Plot.lineY(playerData, { x: "date", y: "rank", stroke: "#d3202a", strokeWidth: 2, curve: "monotone-x" }),
  ...
  const gammaChart = gammaData.length > 0 ? Plot.plot({
    ...
    marks: [
      ...
      Plot.lineY(gammaData, { x: "date", y: "gamma", stroke: "cube", strokeWidth: 2, curve: "monotone-x" }),
  ...
  const gamesChart = Plot.plot({
    ...
    marks: [
      Plot.lineY(playerData, { x: "date", y: "total", stroke: "#8b6914", strokeWidth: 2, curve: "monotone-x" }),
