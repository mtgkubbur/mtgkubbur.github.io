...
//| label: theme-detect
isDark = {
  let dark = document.body.classList.contains("quarto-dark");
  yield dark;
  // Poll for class changes since MutationObserver callback can't yield
  while (true) {
    await new Promise(r => setTimeout(r, 150));
    const now = document.body.classList.contains("quarto-dark");
    if (now !== dark) { dark = now; yield dark; }
  }
}

// Theme-aware color palettes
themeColors = ({
  blue:    isDark ? "#3b9edd" : "#0e68ab",
  green:   isDark ? "#2ecc71" : "#00733e",
  red:     isDark ? "#e74c3c" : "#d3202a",
  gold:    isDark ? "#d4a843" : "#8b6914",
  text:    isDark ? "#c8c4bc" : "#2a1f14",
  muted:   isDark ? "#7a7670" : "#5c4f42",
  grid:    isDark ? "rgba(255,255,255,0.1)" : "#d4cfc9",
  blueArea1: isDark ? "rgba(59,158,221,0.10)" : "rgba(14,104,171,0.12)",
  blueArea2: isDark ? "rgba(59,158,221,0.20)" : "rgba(14,104,171,0.25)"
})
```
...
//| column: screen-inset
{
  ...
  const eloChart = Plot.plot({
    ...
    marks: [
      Plot.areaY(playerData, { x: "date", y1: "score_lower", y2: "score_upper", fill: themeColors.blueArea1, curve: "monotone-x" }),
      Plot.areaY(playerData, { x: "date", y1: "score_q25", y2: "score_q75", fill: themeColors.blueArea2, curve: "monotone-x" }),
      Plot.lineY(playerData, { x: "date", y: "score_median", stroke: themeColors.blue, strokeWidth: 2.5, curve: "monotone-x" }),
      Plot.dot([latest], { x: "date", y: "score_median", fill: themeColors.blue, r: 4 }),
      Plot.text([latest], { x: "date", y: "score_median", text: d => `${d.score_median}`, dy: -12, fontSize: 13, fontWeight: "bold", fill: themeColors.blue })
    ]
  });
  ...
  const winrateChart = Plot.plot({
    ...
    marks: [
      Plot.ruleY([50], { stroke: themeColors.grid, strokeDasharray: "4,4" }),
      Plot.lineY(playerData, { x: "date", y: "winRate", stroke: themeColors.green, strokeWidth: 2.5, curve: "monotone-x" }),
      Plot.dot([latest], { x: "date", y: "winRate", fill: themeColors.green, r: 4 }),
      Plot.text([latest], { x: "date", y: "winRate", text: d => `${d.winRate.toFixed(0)}%`, dy: -12, fontSize: 13, fontWeight: "bold", fill: themeColors.green })
    ]
  });
  ...
  const placementChart = Plot.plot({
    ...
    marks: [
      Plot.lineY(playerData, { x: "date", y: "rank", stroke: themeColors.red, strokeWidth: 2.5, curve: "monotone-x" }),
      Plot.dot([latest], { x: "date", y: "rank", fill: themeColors.red, r: 4 }),
      Plot.text([latest], { x: "date", y: "rank", text: d => `#${d.rank}`, dy: -12, fontSize: 13, fontWeight: "bold", fill: themeColors.red })
    ]
  });
  ...
  const gammaChart = gammaData.length > 0 ? Plot.plot({
    ...
    marks: [
      Plot.ruleY([0], { stroke: themeColors.grid, strokeDasharray: "4,4" }),
  ...
  const gamesChart = Plot.plot({
    ...
    marks: [
      Plot.lineY(playerData, { x: "date", y: "total", stroke: themeColors.gold, strokeWidth: 2.5, curve: "monotone-x" }),
      Plot.dot([latest], { x: "date", y: "total", fill: themeColors.gold, r: 4 }),
      Plot.text([latest], { x: "date", y: "total", text: d => `${d.total}`, dy: -12, fontSize: 13, fontWeight: "bold", fill: themeColors.gold })
    ]
  });
