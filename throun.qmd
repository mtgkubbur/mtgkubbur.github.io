...
# Per-cube records with original cube names (from Google Sheets)
# Use ALL matches for stats (so trophies count games vs non-opted-in players),
# then filter to only display opted-in players.
...
# Trophies: 3+ match wins at a single event (date + cube)
match_results <- games |>
    ...
    ${(() => {
      ...
      return html`
        <details class="cube-detail-toggle">
          <summary>Sundurliðun eftir kubbi</summary>
